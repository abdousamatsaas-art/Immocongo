(() => {
  ...
  // Netlify Admin auth via cookie created by admin-login function
  const ADMIN_COOKIE = 'immocongo_admin_auth';

  const API = {
    annonces: {
      list:   () => fetch('/.netlify/functions/annonces-list').then(r => r.json()),
      create: (data) => fetch('/.netlify/functions/annonces-create', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify(data) }),
      update: (id, data) => fetch('/.netlify/functions/annonces-update', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({id, ...data}) }),
      delete: (id) => fetch('/.netlify/functions/annonces-delete', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({id}) }),
    },
    messages: {
      list:   () => fetch('/.netlify/functions/messages-list').then(r => r.json()),
      delete: (id) => fetch('/.netlify/functions/messages-delete', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({id}) }),
    },
  };
  ...
  const renderAnnouncements = async () => {
    ...
    const all = await API.annonces.list();
  ...
  /* ── Edition ── */
  const startEdit = (id) => {
    // Edition basée sur l'annonce affichée (sinon vide)
    // Récupération via API : on charge la liste et on cherche l'item.
    API.annonces.list().then((list) => {
  ...
  announcementForm?.addEventListener('submit', async (e) => {
    ...
    if (id) {
      await API.annonces.update(id, payload);
    } else {
      await API.annonces.create(payload);
    }
  ...
  const renderFavoris = () => {
    ...
    const all    = [];
    // Favoris admin : on ne stocke plus les annonces en local (via API). On récupère au moment du rendu.
    API.annonces.list().then((list) => {
      const favItems = favIds
        .map(id => Array.isArray(list) ? list.find(a => String(a.id) === id) : null)
        .filter(Boolean);
