...
if (modal.whatsappBtn) modal.whatsappBtn.href = `https://wa.me/242050797071?text=${waText}`;
  ...
  async function loadAnnouncements() {
    try {
      const res = await fetch('/.netlify/functions/annonces-list');
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      if (Array.isArray(data) && data.length) {
        // On garde DEFAULT_PROPERTIES en fallback uniquement si DB est vide
        allAnnouncements = data;
      } else {
        allAnnouncements = DEFAULT_PROPERTIES;
      }
    } catch {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (contactStatus) {
          contactStatus.textContent = 'Message envoyé (mode local). Merci !';
          contactStatus.classList.remove('is-error');
          contactStatus.classList.add('is-success');
        }
        contactForm.reset();
